...
class ListHeroesViewModel(private val repository: RepositoryContract) : BaseViewModel() {
    ...
    private val viewStateResponse: MutableLiveData<ViewStateModel<ArrayList<Hero>>> =
        MutableLiveData()
    private val viewStateResponseLoadMore: MutableLiveData<ViewStateModel<ArrayList<Hero>>> =
        MutableLiveData()
    ...
    fun loadHeroes() {
        ...
        disposables.add(repository.getHeroes().subscribe(
            { base ->
                viewStateResponse.postValue(
                    ViewStateModel(
                        status = ViewStateModel.Status.SUCCESS,
                        model = base.data.results
                    )
                )
            }, { error ->
                viewStateResponse.postValue(
    ...
    fun loadMore(offset: Int) {
        ...
        disposables.add(repository.loadMore(offset).subscribe(
            { base ->
                viewStateResponseLoadMore.postValue(
                    ViewStateModel(
                        status = ViewStateModel.Status.SUCCESS,
                        model = base.data.results
                    )
